...
[center, pos_vector, n] = my_kmean(data_shuffled(1:4, :), num_tr, num_hd, data_Kmean_10(1:4,:));
...
o_Kmean_10_True(o_Kmean_10<0) = -1;
o_Kmean_10_True(o_Kmean_10_True~=-1) = 1;
...
for i=1:length(Test_Data)
    ...
    o_Kmean_10_test(i) = w_10*g;
end

%%
nanmap=isnan(o_Kmean_10_test);
o_Kmean_10_True_Test(o_Kmean_10_test<0) = -1;
o_Kmean_10_True_Test(o_Kmean_10_test>=0) = 1;
o_Kmean_10_True_Test(nanmap)=nan;
% Bimg(nanmap)=nan;
% while ~all(isnan(o_Kmean_10_test(:)))
%     (o_Kmean_10_test<0) = -1;
% end
% o_Kmean_10_True_Test = max(0,o_Kmean_10_test);
% o_Kmean_10_True_Test(o_Kmean_10_test<0) = -1;
%o_Kmean_10_True_Test(o_Kmean_10_test>=0) = 1;
% o_Kmean_10_True_Test(o_Kmean_10_test<0) = -1;
% o_Kmean_10_True_Test(o_Kmean_10_True_Test~=-1) = 1;

%%

% B = reshape(o_Kmean_10_True_Test, 361, 320);
% B = reshape(o_Kmean_10_True_Test, [], 320);
% B = reshape(o_Kmean_10_test, [], 320);
B = reshape(o_Kmean_10_True_Test, [], 361);
B = B';
% %%
% figure
% xplot = Test.XPLOT';
